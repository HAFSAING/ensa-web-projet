...
// Récupérer toutes les questions avec leurs réponses (le cas échéant)
$questions_query = $pdo->query("
    SELECT q.id, q.question, q.date_question, r.reponse, r.date_reponse, m.civilite, m.nom, m.prenom 
    FROM questions q 
...
// Gestion de la soumission d'une nouvelle question
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['question'])) {
    $question_text = htmlspecialchars($_POST['question']);
    $date_question = date('Y-m-d H:i:s');
    
    $stmt = $pdo->prepare("INSERT INTO questions (question, date_question) VALUES (?, ?)");
    $stmt->execute([$question_text, $date_question]);
...
<head>
    ...
    <title>Questions & Réponses - MediStatView</title>
...
<body>
    <style>
        ...
        * {
        ...
        .service-item form button:hover {
...
<header>
    ...
    <section class="hero">
        <div class="container">
            <h2>Vos Questions sur la Santé</h2>
            <p>Posez vos questions de manière anonyme. Nos médecins qualifiés répondront bientôt !</p>
            <div class="cta-buttons">
                <a href="#ask-question" class="btn btn-primary">Poser une question</a>
            </div>
        </div>
    </section>

    <!-- Section Formulaire et Liste des Questions -->
    <section class="services">
        <div class="container">
            <h2>Questions & Réponses</h2>
            <!-- Formulaire pour poser une question -->
            <div class="service-item" id="ask-question">
                <h3>Posez Votre Question</h3>
                <form method="POST">
                    <textarea name="question" rows="5" placeholder="Écrivez votre question ici..." required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px;"></textarea>
                    <button type="submit" class="btn btn-primary">Envoyer la question</button>
                </form>
                <p>Vos données sont protégées et soumises anonymement conformément aux réglementations en vigueur.</p>
            </div>
            ...
            <div class="service-item">
                ...
                <?php else: ?>
                    <?php foreach ($questions as $qa): ?>
                        <div class="qa-item">
                            <h4>Question :</h4>
                            <p><?php echo htmlspecialchars($qa['question']); ?></p>
                            <p><small>Posée le : <?php echo $qa['date_question']; ?></small></p>
                            <?php if ($qa['reponse']): ?>
                                <h4>Réponse :</h4>
                                <p><?php echo htmlspecialchars($qa['reponse']); ?></p>
                                <p><strong>Répondue par :</strong> <?php echo htmlspecialchars($qa['civilite'] . ' ' . $qa['nom'] . ' ' . $qa['prenom']); ?></p>
                                <p><small>Répondue le : <?php echo $qa['date_reponse']; ?></small></p>
                            <?php else: ?>
